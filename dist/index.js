...
function getPlatform() {
  switch (process.platform) {
    ...
    case "win32":
      return "windows";
    default:
      core2.error(`Unsupported platform: ${process.platform}`);
      return;
  }
}
function getArch() {
  switch (process.arch) {
    case "x64":
      return "amd64";
    default:
      core2.error(`Unsupported architecture: ${process.arch}`);
      return;
  }
}
function getVariant() {
...
async function acquirePacker() {
  ...
  if (cachedPath === "") {
    ...
    const cachedPath2 = await tc.cacheDir(extractedPath, "packer", version, variant);
    core2.addPath(cachedPath2);
  } else {
    core2.addPath(cachedPath);
  }
  const packerPath = await io.which("packer", true);
  core2.info(`Version: ${version}`);
  core2.info(`Variant: ${variant}`);
  core2.info(`Cache location: ${packerPath}`);
}
